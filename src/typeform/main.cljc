(ns typeform.main
  "Kotodama WASM entrypoint for the Typeform clean-room actor (L4) — Clojure port.
  ...
  facts (`typeform.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "typeform")
(def tier "L4")
...
(def entity-specs
  [{:entity "Form"     :plural "forms"     :id-prefix "typeform_for"
    :fields [:title :type]                            :required [:title]
    :coerce {}                                        :refs {}}
   {:entity "Field"    :plural "fields"    :id-prefix "typeform_fie"
    :fields [:ref :title :type]                       :required [:ref]
    :coerce {}                                        :refs {}}
   {:entity "Response" :plural "responses" :id-prefix "typeform_res"
    :fields [:token :landedAt :submittedAt :formId]   :required [:token :landedAt]
    :coerce {}                                        :refs {:formId "Form"}}
   {:entity "Answer"   :plural "answers"   :id-prefix "typeform_ans"
    :fields [:fieldId :fieldType :type :value]        :required [:fieldType :type]
    :coerce {}                                        :refs {:fieldId "Field"}}
   {:entity "Webhook"  :plural "webhooks"  :id-prefix "typeform_web"
    :fields [:eventId :eventType :timestamp]          :required [:eventType :timestamp]
    :coerce {}                                        :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"typeform.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "typeform-compat" :tier tier :entities entities} 200])
...
(defn start! [] :typeform-compat/ready)
